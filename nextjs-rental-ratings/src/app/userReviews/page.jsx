...
const fetchReviews = async (user) => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_SERVER}/data/userReviews?user=${user}`);
...
export default function UserReviews() {
    ...
    const handleDelete = async (id) => {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_SERVER}/data/deleteReview?reviewID=${id}`)
            ...
            <ul>
                {Array.from(userReviews).map(rev => {
                    return(<li key={rev.reviewID}>
                        <div id="edit-actions">
                            <button onClick={() => router.replace(`/review-form?id=${rev.reviewID}&user=${searchParams.get('user')}&edit=true`)}>Edit</button>
                            <button onClick={() => handleDelete(rev.reviewID)}>Delete</button>
                        </div>
